...
STAGE_CONFIGS: dict[str, dict] = {
    ...
    "s04_tool": {
        ...
        "cost_hint": "low",
        "description_en": "Select tools. Only tools whitelisted in selected_tools are eager (callable immediately); the rest are deferred — LLM must call ToolSearch(names=[...]) to load schemas (Claude Code style progressive disclosure, v1.4.0). RAG collections are exposed as the rag_search builtin (R3 — LLM calls it directly instead of stage auto-search).",
        "icon": "🔧",
        "fields": [
        ...
        "behavior": [
            ...
            "selected_tools 화이트리스트 기준 분리 — eager (호출 가능) / deferred (이름+1줄만, schema 숨김)",
            "eager 도구는 Anthropic API tools= 인자에 full schema 박힘 → LLM 즉시 호출 가능",
            "deferred 도구는 system_prompt 의 <deferred_tools> 섹션에 이름+1줄 desc 만 노출",
            "LLM 이 ToolSearch(names=['x','y']) 빌트인 호출 시 schema 가 state.tool_definitions 에 합류 → 다음 turn tools= 누적 (dynamic catalogue)",
            "search_tools 빌트인: 카탈로그 ≥ 12 시 키워드 검색 (Claude Code 'select:' / keyword 둘 다)",
            "discover_tools 빌트인: 특정 도구 schema 조회만 (호출 가능 상태 X)",
    ...
    "s06_context": {
        ...
        "behavior": [
            "Cascade 압축 (v1.4.0 default 1개 strategy) — 압력별 L3→L4→L5 자동 에스컬레이션 (Claude Code 5-Level)",
            "L3 microcompact: 오래된 tool_result 만 placeholder 교체. 원본은 pd_stores['tool_result'] 에 보관",
