...
산출물 구조::
    ...
    │       "dependencies": {
    │         "xgen-harness-engine-node": "^0.28.0"
    │       }
    │     }
    ├── bin/cli.js   — `require("xgen-harness-engine-node").serve(spec)` thin wrapper
...
DEFAULT_ENGINE_DEP = "0.28.0-alpha.1"
 ...
 *
 * xgen-harness-engine-node 가 spec.json 을 읽어 13 stage / 31 strategy /
...
let engine;
try {
  engine = require("xgen-harness-engine-node");
} catch (e) {
  console.error(
    "[xgen-harness] xgen-harness-engine-node not installed. " +
    "Run `npm install xgen-harness-engine-node` first."
  );
...
- Engine: `xgen-harness-engine-node` reads the spec and runs the same 13-stage
